...
internal sealed class Class252(uint uint_4, uint uint_5, uint uint_6, string string_3) : GClass541
{
    public static string string_1;

    private readonly uint uint_0 = uint_4;

    private readonly uint uint_1 = uint_5;

    private uint uint_2;

    private readonly uint uint_3 = uint_6;
    ...
    private string String_4 => GClass486.smethod_5(uint_3);

    [SpecialName]
    public override string vmethod_0()
    {
        string text = ((uint_3 == 0) ? "" : (" " + String_4));
        return "copy_node " + String_1 + " " + String_2 + text + " " + String_3;
    }
    ...
    public override GClass542 vmethod_2(VaultDatabase gclass478_0)
    {
        ...
        if (uint_3 == 0)
        {
            gclass537_ = gClass2.method_10(gClass2, string_2, gClass3);
        }
        else
        {
            GClass537 gClass4 = gClass2.method_2(uint_3);
            if (gClass4 == null)
            {
                gClass.method_1("The class '" + String_1 + "' does not contain node '" + String_4 + "'.");
                return gClass;
            }
            gclass537_ = gClass2.method_10(gClass4, string_2, gClass3);
        }
